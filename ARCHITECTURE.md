...
# 1. 标题不含 AUXILIARY_MARKERS 关键词的导读章节会被错标(如"庄子的艺术特色")

# 2. chunk 第一行不是 heading 但第二行是 heading 的情况会被忽略

# (如《庄子》逍遥游题解,第一行是日期"二〇一〇年一月",第二行才是 ### 【题解】)

# 应对:这两类情况罕见且危害有限,通过评估标注区分"事实/开放"绕过
